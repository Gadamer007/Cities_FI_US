...
def create_scatter_plot(data, reference_city):
    ...
    fig = px.scatter(
        ...
        x='Col_Diff_%',
        y='Sal_Diff_%',
        text='City_Short',  # Display short city name on the plot
        hover_data={'City': True, 'Col_Diff_%': ':.1f', 'Sal_Diff_%': ':.1f'},  # Include full city name in hover info
        labels={
    ...
    fig.update_layout(
        ...
        width=1200,  # Increased width for a more rectangular aspect ratio
        legend=dict(
